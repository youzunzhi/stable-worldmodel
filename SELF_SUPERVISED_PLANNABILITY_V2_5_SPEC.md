# Self-Supervised Plannability v2.5 experiment specification

Status: frozen implementation and launch contract; not an experiment result

Protocol ID: `self-supervised-plannability-v2.5`

Short name: `SSP-v2.5`

Source branch: `codex/self-supervised-plannability-v2.5`

Tasks promoted to formal training: `tworoom`, `pusht`, `cube`, `reacher`

Superseded protocol: `self-supervised-plannability-v2`
...
SSP-v2.5 does **not** test whether an arbitrary geometry exists in a continuous
...
All four task thresholds are user-registered SSP constants in mean-MSE units:
PushT `1.5`, Cube `1.0`, TwoRoom `1.5`, and Reacher `0.7`. They are not
retuned from SSP-v2.5 training, validation, held-out, or CLEAR observations.
Cube is included by explicit four-task registration, but its earlier
`find-goal-threshold` study had no feasible calibrated operating point. Cube
therefore remains a formal result only for this fixed SSP-v2.5 verifier
contract; it is not evidence that the verifier is semantically calibrated.

### 1.1 v2.5 correction boundary

SSP-v2 compared the mean-MSE-valued thresholds above against a 192-coordinate
sum-SSE. SSP-v2.5 corrects that unit mismatch by changing only the immutable
binary verifier to float32 mean-MSE. Equivalently, the thresholds in the old
sum-SSE coordinate system would be PushT `288`, Cube `192`, TwoRoom `288`,
and Reacher `134.4`.

The rotated CEM ranking cost remains in its original quadratic sum scale;
positive global scaling does not change candidate ordering. SSP-v2 artifacts
are retained as superseded evidence and are never reused as v2.5 training,
profile, or CLEAR evidence.
...
```text
y(A) = 1[min_{h=1..5} mean_D((z_hat_h(A) - z_g)^2) < epsilon_task].
```

The comparison is strict `<`; accumulation is float32 mean-MSE over all 192
latent coordinates. Thresholds are PushT `1.5`, Cube `1.0`, TwoRoom `1.5`,
and Reacher `0.7`.
...
the v2.5 controller output. Candidate, elite, archive, and returned-hit fields
...
SSP-v2.5 retains the action-effect-aligned rotated metric introduced in v2.
Preparation takes the 800 training pairs and their recorded
...
optimizer replicates. Identity and learned cells must both use the v2.5 rotated
...
For TwoRoom, PushT, Cube, and Reacher:
...
evidence. Stable failures include `SSP_V2_5_INPUT_HASH_MISMATCH`,
`SSP_V2_5_CRN_MISMATCH`, `SSP_V2_5_FROZEN_MODEL_MUTATION`,
`SSP_V2_5_DIAGNOSTIC_FAILED`, `SSP_V2_5_INCOMPLETE`, and
`SSP_V2_5_FORMAL_ROOT_EXISTS`.
