...
def _summarize_episode(arrays: dict[str, np.ndarray]) -> dict[str, Any]:
    ...
    success = (
        np.nan_to_num(arrays['success'].astype(float).reshape(-1), nan=0.0)
        > 0.5
    )
    success_event = success & ~np.concatenate([[False], success[:-1]])
    truncated = (
        np.nan_to_num(arrays['truncated'].astype(float).reshape(-1), nan=0.0)
        > 0.5
    )
    terminated = (
        np.nan_to_num(arrays['terminated'].astype(float).reshape(-1), nan=0.0)
        > 0.5
    )
    ...
    target_yaw = (
        arrays['privileged_target_block_yaw'].astype(float).reshape(-1)
    )
    target_yaw_delta = np.abs(np.angle(np.exp(1j * np.diff(target_yaw))))
    ...
    action_tv = np.linalg.norm(
        np.diff(action, axis=0)[valid_action_pair], axis=1
    )
...
def scan_episode_metrics(
    ...
    for batch_start in range(episode_start, episode_end, episodes_per_take):
        ...
        for local_ep in range(batch_start, batch_end):
            ...
            episode = {
                name: value[start:end] for name, value in arrays.items()
            }
...
def choose_representatives(
    ...
    def medoid(pool: list[dict[str, Any]], fields: tuple[str, ...]):
        ...
        index = int(
            np.argmin(np.linalg.norm((matrix - center) / scale, axis=1))
        )
        return pool[index]
    ...
    slow_pool = [
        m for m in successful if m['first_success_step'] >= late_threshold
    ]
    slow = max(
        slow_pool,
...
def load_episode(
    dataset: Any, episode_idx: int, rows_per_episode: int
) -> dict[str, Any]:
    ...
    for name in VIDEO_COLUMNS:
        ...
        result[name] = (
            column.to_pylist() if name == 'pixels' else _as_numpy(column)
        )
    return result
...
def load_episode_from_dataset_server(
    ...
    if any(int(row['episode_idx']) != episode_idx for row in rows):
        raise RuntimeError(
            'dataset server response crossed an episode boundary'
        )
...
def _draw_polyline(
    ...
    if len(points) < 2:
        return
    draw.line(
        [_xy_to_canvas(p[:2], box) for p in points], fill=fill, width=width
    )
...
def render_video(
    ...
    success = (
        np.nan_to_num(episode['success'].astype(float).reshape(-1), nan=0.0)
        > 0.5
    )
    success_event = success & ~np.concatenate([[False], success[:-1]])
    contact = np.nan_to_num(
    ...
    target_change_steps = (
        np.flatnonzero(np.linalg.norm(np.diff(target, axis=0), axis=1) > 1e-5)
        + 1
    )
    ...
    try:
        for i, blob in enumerate(episode['pixels']):
            ...
            draw.rectangle(
                (x0, y0, x0 + pw, y0 + ph), outline='#334155', width=2
            )
            ...
            draw.text(
                (x0 + 10, y0 + 8), 'cube', font=fonts[16], fill='#ff7b86'
            )
            draw.text(
                (x0 + 72, y0 + 8), 'target', font=fonts[16], fill='#ffd166'
            )
            draw.text(
                (x0 + 148, y0 + 8),
                'end-effector',
            ...
            for j, name in enumerate(ACTION_NAMES):
                yy = action_y + j * 32
                draw.text(
                    (680, yy), f'{name:>4}', font=fonts[16], fill='#cbd5e1'
                )
                center = 812
                draw.line(
                    (center - 105, yy + 10, center + 105, yy + 10),
                    fill='#475569',
                    width=2,
                )
                ...
                draw.rectangle(
                    (min(center, end), yy + 4, max(center, end), yy + 16),
                    fill=color,
                )
            ...
            draw.line(
                (timeline_x0, timeline_y, timeline_x1, timeline_y),
                fill='#475569',
                width=4,
            )
            progress_x = timeline_x0 + int(
                i / (len(step) - 1) * (timeline_x1 - timeline_x0)
            )
            draw.line(
                (timeline_x0, timeline_y, progress_x, timeline_y),
                fill='#60a5fa',
                width=5,
            )
            for s in np.flatnonzero(success_event):
                sx = timeline_x0 + int(
                    s / (len(step) - 1) * (timeline_x1 - timeline_x0)
                )
                draw.ellipse(
                    (sx - 4, timeline_y - 4, sx + 4, timeline_y + 4),
                    fill='#86efac',
                )
            for s in target_change_steps:
                ...
                draw.line(
                    (sx, timeline_y - 8, sx, timeline_y + 8),
                    fill='#ffd166',
                    width=2,
                )
            draw.text(
                (1015, 650),
                'success / target-change timeline',
                font=fonts[16],
                fill='#cbd5e1',
            )
...
def make_contact_sheet(
    ...
    draw.text(
        (30, 22),
        'Representative Cube training trajectories',
        font=font_title,
        fill='#f5f7fa',
    )
    for index, (label, summary) in enumerate(representatives.items()):
        episode = load_episode(
            dataset, summary['episode_idx'], rows_per_episode
        )
        ...
        for j, s in enumerate(key_steps):
            ...
            draw.text(
                (x + 8, y + 198), f'step {s}', font=_font(16), fill='#ffffff'
            )
        draw.text(
            (735, y + 50),
...
def main() -> None:
    ...
    if args.render_episode is not None:
        if args.render_label is None:
            raise ValueError(
                '--render-label is required with --render-episode'
            )
        summary = None
        ...
        if summary is None:
            raise ValueError(
                f'episode {args.render_episode} not found in metrics'
            )
        episode = (
    ...
    if args.scan_start is not None or args.scan_end is not None:
        if (
            args.scan_start is None
            or args.scan_end is None
            or args.scan_output is None
        ):
            raise ValueError(
    ...
    last_episode = (
        dataset.take([row_count - 1], columns=['episode_idx'])
        .column('episode_idx')[0]
        .as_py()
    )
        ...
        'source': {
            ...
            'identity_anchor': {
                ...
                'absolute_difference_m': abs(
                    anchor_distance - 0.06859241639835252
                ),
            },
        },
    ...
    if not args.skip_videos:
        for label, summary in representatives.items():
            ...
            render_video(
                ...
                label,
                args.output_dir
                / f'{label}_episode_{summary["episode_idx"]}.mp4',
                args.fps,
            )
        make_contact_sheet(
            dataset,
    ...
    print(
        json.dumps(
            {'audit': str(audit_path), 'representatives': representatives},
            indent=2,
        )
    )
