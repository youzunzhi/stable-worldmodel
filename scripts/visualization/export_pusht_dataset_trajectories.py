...
def summarize_episode(
    ...
    return {
        ...
        'block_net_displacement_px': float(
            np.linalg.norm(block[-1] - block[0])
        ),
        ...
        'agent_velocity_norm_mean': float(
            np.linalg.norm(velocity, axis=1).mean()
        ),
        'action_norm_mean': float(
            np.linalg.norm(action[valid_action], axis=1).mean()
        ),
...
def _plot_xy(
    point: np.ndarray, box: tuple[int, int, int, int]
) -> tuple[int, int]:
...
def render_video(
    ...
    try:
        for idx, raw_pixels in enumerate(pixels):
            ...
            draw.text(
                (x0 + 8, y0 + 7), 'agent', font=fonts[15], fill='#4cc9f0'
            )
            draw.text(
                (x0 + 70, y0 + 7), 'block', font=fonts[15], fill='#ff6b7a'
            )
            draw.text(
                (x0 + 130, y0 + 7),
                'clean block',
            ...
            for action_idx, name in enumerate(('dx', 'dy')):
                ...
                endpoint = int(
                    center + 100 * np.clip(action[idx, action_idx], -1, 1)
                )
            ...
            progress_y = 690
            draw.line(
                (680, progress_y, 1228, progress_y), fill='#4b5563', width=3
            )
...
def make_contact_sheet(
    previews: list[tuple[str, Any]], output_path: Path
) -> None:
    from PIL import Image, ImageDraw
    ...
    for idx, (label, preview) in enumerate(previews):
        ...
        ImageDraw.Draw(tile).text(
            (14, 11), label, font=_font(17), fill='white'
        )
...
def main() -> None:
    ...
    with h5py.File(args.dataset, 'r') as h5:
        ...
        with (args.output_dir / 'pusht_episode_metrics.jsonl').open(
            'w'
        ) as file:
            for row in metrics:
        ...
        if not args.skip_videos:
            ...
            for label, summary in representatives.items():
                ...
                output_path = (
                    args.output_dir / f'{label}_episode_{episode_idx}.mp4'
                )
            ...
            make_contact_sheet(
                previews,
                args.output_dir
                / 'representative_noise_sweep_contact_sheet.png',
            )
    ...
    print(
        json.dumps(
            {
                'audit': str(
                    args.output_dir / 'pusht_training_data_audit.json'
                ),
