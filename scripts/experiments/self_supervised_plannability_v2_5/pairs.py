...
def _action_statistics(actions: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if values.ndim < 2:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'action column must be at least 2D'
        )
    ...
    if not np.isfinite(mean).all() or not np.isfinite(std).all():
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'non-finite action statistics'
        )
    if np.any(std <= 0):
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'action standard deviation is zero'
        )
...
def prepare_task(
    *,
    ...
    """Create immutable v2.5 pair, action, latent, and basis artifacts."""
    import h5py
    ...
    if formal and config['task'] not in FORMAL_TASKS:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH',
            f'{config["task"]} is diagnostic-only in the v2.5 contract',
        )
    ...
    protocol = {
        ...
        'verifier': 'trajectory-aware float32 mean-MSE strict binary',
    ...
    if before_model_hash != after_model_hash:
        raise SSPV25Failure(
            'SSP_V2_5_FROZEN_MODEL_MUTATION',
            'model changed during v2.5 preparation',
        )
    selected_pair_ids = [
        row['pair_id'] for split in SPLITS for row in chosen[split]
    ]
    if len(selected_pair_ids) != len(set(selected_pair_ids)):
        raise AssertionError('pair IDs overlap across v2.5 splits')
    split_group_sets = [
    ...
    if any(
        ...
        raise AssertionError('leakage groups overlap across v2.5 splits')
    overlap = {
        split: sorted(
