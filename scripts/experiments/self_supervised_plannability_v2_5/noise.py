"""Keyed common-random-number schedules and v2.5 hit-funnel observer."""
...
from .contracts import SSPV25Failure, sha256_json
    ...
    def __call__(
        ...
        *,
        ...
        if self.generator is None:
            ...
            raise SSPV25Failure(
                'SSP_V2_5_CRN_MISMATCH', 'noise device changed mid-solve'
            )
        value = torch.randn(
            *shape,
...
class HitFunnelObserver:
    ...
    def start_batch(self, *, start_idx: int, end_idx: int) -> None:
        if start_idx != 0 or self.population_hit_counts:
            raise ValueError('SSP-v2.5 planner requires one batch per solve')
    ...
    def __call__(
        ...
        if batch_start != 0 or step != len(self.population_hit_counts):
            raise ValueError('unexpected v2.5 observer ordering')
