"""Run one matched-code-path secondary SSP-v2.5 CLEAR cell."""
...
def run_clear_cell(
    *,
    ...
    for path in (basis_path, action_stats_path):
        if not path.is_file():
            raise SSPV25Failure(
                'SSP_V2_5_INCOMPLETE', f'missing preparation artifact {path}'
            )
    ...
    overrides = [
        ...
        'seed=42',
        'ssp.version=25',
        f'ssp.threshold={config["epsilon_task"]}',
    ...
    if geometry in {'identity', 'identity-repeat'}:
        ...
        if seed not in config['replicate_seeds']:
            raise ValueError(f'unknown v2.5 geometry replicate {geometry}')
    ...
    if (
        ...
        raise SSPV25Failure(
            'SSP_V2_5_INCOMPLETE', 'CLEAR completion/protocol audit failed'
        )
    expected_mode = (
        'identity-zero-theta'
        if geometry in {'identity', 'identity-repeat'}
    ...
    if ssp.get('geometry_mode') != expected_mode:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'CLEAR geometry mode mismatch'
        )
    return result
