"""One-shot held-out SSP-v2.5 returned-hit profile with uncertainty."""
...
def evaluate_profile(
    *,
    ...
    if not completion_path.is_file() or not selected_path.is_file():
        raise SSPV25Failure('SSP_V2_5_INCOMPLETE', 'training is not complete')
    ...
    if checkpoint.get('protocol_id') != PROTOCOL_ID:
        raise SSPV25Failure(
            ...
            'selected geometry protocol mismatch',
        )
    ...
    if len(pair_ids) != LOCKED['test_pairs']:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'test pair count is not locked'
        )
    groups = _group_ids(
    ...
    if before != after:
        raise SSPV25Failure(
            'SSP_V2_5_FROZEN_MODEL_MUTATION', 'model changed during profile'
        )
    summary = {
        'protocol_id': PROTOCOL_ID,
        'task': config['task'],
