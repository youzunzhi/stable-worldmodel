"""Frozen SSP-v2.5 constants and auditable artifact helpers."""
...
PROTOCOL_ID = 'self-supervised-plannability-v2.5'
FORMAL_TASKS = ('tworoom', 'pusht', 'cube', 'reacher')
DIAGNOSTIC_TASKS = ()
...
THRESHOLD_REGISTRATIONS = {
    task: {
        'source': 'user-registered SSP task threshold',
        'registered_value': value,
        'registered_unit': 'float32 mean_D((z_i-z_j)^2)',
        'registered_before_ssp_v2_5_outcomes': True,
        'ssp_verifier_distance': ('float32 mean_D((predicted-goal)^2)'),
        'sum_sse_equivalent': round(value * 192, 10),
        'comparison': '<',
    }
...
class SSPV25Failure(RuntimeError):
    """Structured terminal failure with a stable v2.5 protocol code."""
...
def validate_config(config: dict) -> None:
    ...
    if float(config.get('epsilon_task')) != THRESHOLDS[task]:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'task threshold is not locked'
        )
    if config.get('threshold_registration') != THRESHOLD_REGISTRATIONS[task]:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH',
            f'{task} threshold registration is not locked',
        )
...
def create_root(path: str | Path, *, formal: bool) -> Path:
    ...
    try:
        ...
        code = (
            ...
            if formal
            else 'SSP_V2_5_OUTPUT_ROOT_EXISTS'
        )
        raise SSPV25Failure(
