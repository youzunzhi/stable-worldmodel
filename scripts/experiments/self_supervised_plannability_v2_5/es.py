"""Orthogonal antithetic ES, diagnostics, and selection for SSP-v2.5."""
...
def _load_preparation(preparation_dir: str | Path, config: dict) -> dict:
    ...
    if not summary_path.is_file():
        raise SSPV25Failure('SSP_V2_5_INCOMPLETE', f'missing {summary_path}')
    summary = json.loads(summary_path.read_text())
    if summary.get('protocol_id') != PROTOCOL_ID:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'preparation protocol mismatch'
        )
    if summary.get('task') != config['task']:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'preparation task mismatch'
        )
    if sha256_file(root / 'pair_latents.pt') != summary['pair_latents_sha256']:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'pair latent hash mismatch'
        )
    if (
        ...
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH', 'action-effect basis hash mismatch'
        )
...
def _validate_crn(
    ...
    if len(left) != len(right):
        raise SSPV25Failure('SSP_V2_5_CRN_MISMATCH', 'result lengths differ')
...
def run_smoke(
    *,
    ...
    if before != after:
        raise SSPV25Failure(
            'SSP_V2_5_FROZEN_MODEL_MUTATION', 'model changed during diagnostic'
        )
    summary = {
        'protocol_id': PROTOCOL_ID,
...
def run_training(
    *,
    ...
    """Run one create-only formal SSP-v2.5 training replicate."""
    ...
    if len(train_ids) != 800 or len(validation_ids) != 256:
        raise SSPV25Failure(
            'SSP_V2_5_INPUT_HASH_MISMATCH',
            'prepared pair counts are not locked',
        )
    ...
    for outer_step, batch in enumerate(train_batches, start=1):
        ...
        if not torch.isfinite(center).all():
            raise SSPV25Failure(
                'SSP_V2_5_INCOMPLETE', 'non-finite optimizer center'
            )
    ...
    if before_model_hash != after_model_hash:
        raise SSPV25Failure(
            'SSP_V2_5_FROZEN_MODEL_MUTATION', 'model changed during training'
        )
    completion = {
        'protocol_id': PROTOCOL_ID,
        'task': config['task'],
        'replicate_seed': replicate_seed,
        'terminal_code': 'SSP_V2_5_COMPLETED',
